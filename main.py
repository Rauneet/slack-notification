...
def calculate_sleep_duration():
    """
    # Calculates the time in seconds the script should sleep before the next action is required.
    
    # This function determines the minimum sleep duration based on two events:
    # 1. The next periodic check time, which occurs every 15 minutes.
    # 2. The daily summary time, scheduled for 18:00 each day.

    # The function first calculates when the next 15-minute check should occur, rounding down to remove any seconds or microseconds for precision.
    # It then determines if the next summary time (18:00) has already passed for the day. If it has, it schedules the next summary time for 18:00 the next day.
    # The function returns the lesser of the two durations (next check time or next summary time) to ensure the script awakens to perform the necessary action efficiently and on time.

    # Returns:
    #     sleep_duration (float): The number of seconds the script should sleep before the next scheduled check or summary.
    """
    current_time = datetime.datetime.now(TIMEZONE)
